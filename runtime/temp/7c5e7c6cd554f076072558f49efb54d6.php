<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:68:"E:\project\tptianyu\public/../application/admin\view\order\read.html";i:1547465114;}*/ ?>
<!--_meta 作为公共模版分离出去-->
<!DOCTYPE HTML>
<html>
<head>
...
<article class="cl pd-20">
	<table class="table table-border  table-bg">
				<thead>
					<tr>
						<th scope="col" colspan="5">单子详情 <?php echo !empty($order['received'])?"<span class='c-success r'> 已收款 ￥" .$order['realmoney'] . "</span>" : ""; ?></th>
					</tr>
				</thead>
				<tbody>
					<tr>
						<td><b>日期：</b><?php echo date('Y-m-d',$order['date']); ?></td>
						<td><b>副本名称：</b><?php echo $order['game_name']; ?></td>						
						<td><b>总金额：</b> ￥ <?php echo $order['total']; ?></td>						
						<td><b>人数：</b><?php echo $order['member']; ?></td>						
						<td><b>平均工资：</b> ￥ <?php echo $order['salary']; ?></td>
					</tr>
					<tr>
						<td><b>区服：</b><?php echo $order['area']; ?></td>
						<td><b>老板：</b><?php echo $order['boss']; ?></td>						
						<td colspan="3"><b>备注：</b><?php echo $order['remark']; ?></td>
					</tr>
					<tr>
						<td><b>实收金额：</b> ￥ <?php echo $order['realmoney']; ?></td>
						<td><b>工资总额：</b><?php echo $order['salary_tot']; ?></td>						
						<td colspan="3"><b>状态：</b><?php echo !empty($order['received'])?'<span class="c-green">已收款</span>' : '<span class="c-red">未收款</span>'; ?></td>
					</tr>
				</tbody>
	</table>
	<table class="table table-border  table-bg">
				<thead>
					<tr>
						<th scope="col" colspan="2">提成信息<span class="c-warning r"> 总提成：￥<?php echo $order['bonus_tot']; ?></span></th>
					</tr>
				</thead>
				<tbody>
				<?php foreach($bonus as $b): ?>
					<tr>
						<td>提成：<?php echo $b['name']; ?></td>
						<td>金额： ￥ <?php echo $b['money']; ?></td>
					</tr>
				<?php endforeach; ?>
				</tbody>
	</table>
	...
	<table class="table table-border table-bg table-hover">
				<thead>
					<tr>
						<th scope="col" colspan="8">工资详情<span class="c-warning r"> 全部工资：￥<?php echo $salary_tot; ?>&emsp;未发工资：￥<?php echo $salary_surplus; ?></span></th>
					</tr>
					<tr>
						<th width="100">昵称</th>
						<th width="100">工资</th>
						<th>状态</th>
						<?php if((\think\Session::get('manager_info.role_id') <= 4)): ?>
						<th>操作</th>
						<th width="100">修改</th>
						<th width="100">按钮</th>
						<?php endif; ?>
				...
				<?php foreach($salary as $s): ?>
					...
					<tr class="salary_tr">
						<td salary_id="<?php echo $s['id']; ?>" status="<?php echo $s['status']; ?>" new_salary="0"><?php echo $s['name']; ?></td>
						<td class="salary_td"> ￥ <?php echo $s['salary']; ?></td>
						<td><?php echo !empty($s['status'])?'<span class="c-green">已发放</span>' : '<span class="c-red">未发放</span>'; ?></td>
						<?php if((\think\Session::get('manager_info.role_id') <= 4)): ?>
						<td>
							<?php if(($s['status'] == 0)): ?>
							<input type="button" class="btn btn-secondary-outline radius size-MINI" value="修改" onclick="salary_edit_button(this,<?php echo $s['id']; ?>);">
							<input type="button" class="btn btn-success-outline radius size-MINI" value="发放" onclick="salary_is_status(<?php echo $s['id']; ?>,this,<?php echo $s['status']; ?>);">
						</td>
							<?php endif; ?>
						<td></td>
						<td></td>
...
function salary_is_status(id,obj,status){
	layer.confirm('确认提前发放工资吗？',function(index){
		...
		/*发送ajax*/
		$.ajax({
			'url' : "<?php echo url('admin/order/edit_salary_status'); ?>",
			'type': 'post',
...
function salary_edit_button(obj,id){
	...
	/*显示计算按钮*/
	$('#surplus').html('工资总额： ￥ ' + <?php echo $salary_surplus; ?>);
	$('#surplus').attr('surplus_value',<?php echo $salary_surplus; ?>);
	$('#salary_submit').show();
}
/*工资修改确定按钮*/
...
function surplus_average(){
	...
	/*计算新的剩余工资*/
	surplus_fuc(<?php echo $order['id']; ?>,attr_new_salary,salary_id,'20004');
}
